...
export default function ProductDetail ({$target, initialState}) {
  ...
  // fetchProduct 이후 화면이 렌더링 되었을 때 동작할수 있도록 let으로 생성만 해둔다.
  ...
  // 화면 깜빡임 방지하기(플래그 변수 하나를 두어서 초기 1회에만 렌더링이 되도록 처리)
  ...
  $productDetail.addEventListener('change', e => {
    ...
    if (e.target.tagName === 'SELECT') {
      // 상품 옵션을 나타내는 option의 value에는 optionId를 담고 있음.
      // 이를 가져와서 숫자값을 바꾼다.
